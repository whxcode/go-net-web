...
export default function Contacts() {
  ...
  const searchUsers = async (inputValue?: string) => {
    ...
    try {
      ...
      alert(
        `${t("contacts.search_failed")}: ${error?.message || t("common.error")}`,
      );
    }
  };
  ...
  const createTag = async () => {
    ...
    try {
      const res = await post<Tag>("/api/tags", {
        name: newTagName,
        color: newTagColor,
      });
    ...
    return (
      <div className="page" id="contacts-page">
        <div className="page-header">
          <button className="back-btn" onClick={() => setEditingTag(null)}>
            <ChevronLeft size={20} />
          </button>
          ...
          {friends.map((f) => {
            ...
            return (
              <div
                key={f.id}
                className="list-item"
                onClick={() => toggleFriendTag(editingTag.id, f.id)}
  ...
  if (filterTag) {
    ...
    return (
      <div className="page" id="contacts-page">
        <div className="page-header">
          <button className="back-btn" onClick={() => setFilterTag(null)}>
            <ChevronLeft size={20} />
          </button>
          <h1 style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span
  ...
  const ftm = friendTagMap();
  const groupedSections: { tag: Tag; friends: Friend[] }[] = tags
    .map((tag) => ({
      tag,
      friends: friends.filter((f) =>
        (assignments[String(tag.id)] || []).includes(f.id),
      ),
    }))
            ...
            {tags.length > 0 && (
              ...
              >
                <span style={{ fontSize: 13, color: "var(--text-muted)" }}>
                  <Tag size={16} /> {t("tags.group_by_tag")}
                </span>
                <div
          ...
          <>
            ...
            {groups.map((g) => (
              <div
                key={g.id}
                className="list-item"
                onClick={() => navigate(`/chat/${g.id}?group=1`)}
              >
                      ...
                      >
                        <div
                          style={{
                            ...
                            height: 22,
                            borderRadius: 6,
                            border: selectedMembers.has(f.id)
                              ? "none"
                              : "2px solid var(--border)",
                            background: selectedMembers.has(f.id)
                              ? "var(--accent)"
                              : "transparent",
                            display: "flex",
                            alignItems: "center",
